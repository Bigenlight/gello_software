...
>
> 리포 여러 곳에 D435 시리얼이 **두 쌍** 등장한다. **그 둘은 같은 카메라 2대의 서로 다른
> 시리얼 필드다.** 카메라는 한 번도 교체된 적이 없고, 쌍이 뒤집힌 적도 없다.
>
> 같은 물리 USB 포트에서 두 값이 동시에 나온다 (2026-07-29 실측):
>
> | 물리 포트 | `camera_info.serial_number` | `camera_info.asic_serial_number` | 장치 |
> |---|---|---|---|
> | `4-4.1` | **`147122072740`** | `151623020789` | D435 → cam1 |
> | `4-4.3` | **`243222072700`** | `322743060038` | D435IF → cam2 |
>
> **`serial_no:=`가 매칭하는 것은 `serial_number` 쪽이다.** 직접 확인했다:
>
> ```
> rs.config().enable_device('151623020789')  ->  NO MATCH     # ASIC 시리얼
> rs.config().enable_device('147122072740')  ->  MATCHED      # device 시리얼
> ```
>
> 커널 USB 디스크립터(`journalctl`, `/sys/bus/usb/devices/*/serial`)는 **ASIC 시리얼**을
> 노출한다. 그래서 저널을 grep하면 `151623020789`/`322743060038`만 보이고
> `147122072740`/`243222072700`은 0회가 나온다 — **이건 "그 카메라를 본 적이 없다"는 뜻이
> 아니라 "커널은 다른 필드를 보여준다"는 뜻이다.** 2026-07-28 판이 이 둘을 혼동해
> "옛 쌍은 이 머신이 열거한 적 없다"고 결론지었고, `607e541`이 기본값을 **ASIC 시리얼로
> 바꿨다. 그 값으로는 realsense2_camera가 장치를 절대 찾지 못한다** — 위 `NO MATCH`가 그것이다.
> 자동 해석이 들어오면서 결과적으로 가려졌지만, 원인은 하드웨어 교체가 아니라 **필드 혼동**이었다.
>
