...
### 현황 요약 (2026-07-29 판정 + **2026-07-30 오전 G24 실기 PASS · 신규 G25~G29 · 2026-07-30 오후 백그라운드 추종 스레드(`edbb3f5`/`d6965a9`) 신규 G32/G33/G34**, 머지 `3f199d4` 이후)
...
| **G24** | **개입 손맛 — 타깃 갱신율이 낮아 매 주기 가속/제동/정지** | (미발견) | 🟡 **창 밖까지 해결 (07-30 오후, `edbb3f5`)** — 추종이 데몬 스레드로 나가 창 밖 HOLD 27.7 %→0, 데드맨 release 512→33 ms. 조작자 "개입 속도는 좀 고쳐졌어". **단 background 경로의 체계적 실기 검증은 아직 없다**(오전 3 run은 in-window 경로). 예산 제거의 대가 → G33, mock 루프 0회 |
...
- 🔴 **2026-07-30 오후부터 이 갭의 무게가 커졌다.** 개입 경로에서 `InterventionBudget`이
  제거돼(→ G24) **박스가 사람 개입의 유일한 위치 상한**이 됐다. governor는 속도 상한이지
  위치 상한이 아니다. 그래서 박스가 꺼진 `run_real_hil.py`로 `--arm` 하는 것은 이제
  production actor보다 **덜 안전하다** → **G34**.
...
### 🔧 2026-07-30 오후 (`edbb3f5`) — **스레드 안전해졌다.** 확증된 결함 3건이 고쳐졌다

백그라운드 추종 스레드(`04` §10)가 들어오면서 이 컨트롤러를 **두 스레드가 부르게 됐고**,
적대적 검수가 **실제 컨트롤러 + `ur_kin`으로 재현해** 다음 3건을 확증했다. 셋 다
`4197f5b` 이전부터 잠재해 있던 것이 아니라 **다중 스레드에서만 드러나는 것**이지만,
숫자가 있으므로 기록한다(📌 2026-07-30 오후, 오프라인 재현):

| # | 결함 | 실측 | 왜 위험한가 |
|---|---|---|---|
| 1 | `(T_cmd, q_cmd)`가 **서로 다른 틱의 것** | 108,949 중 **12,143 = 11.1 %** | 두 값이 따로 저장돼 있고 `_integrate`가 `self.T_cmd`를 **세 번 따로** 읽었다. 다른 틱의 `T`에서 나온 `q`를 IK seed로 쓰면 **브랜치 연속성이 깨진다.** 업샘플러는 속도만 제한하므로 그 결과가 **10초짜리 눈먼 한 바퀴 스윕**으로 실행된다 |
| 2 | `_hold()`가 "직전 명령 재발행"이라고 선언하고 `self.q_cmd`를 반환 | 130,028 HOLD 틱 중 **25개**가 직전 명령과 달랐고 최대 **0.2047 rad**(`dq_step_max`의 **3.3배**) | 두 스레드에서 `self.q_cmd`는 **상대가 갈아치운 값**이다. **GUI가 HOLD를 표시하는 동안 팔이 움직인다** |
| 3 | `dq_step_max` 게이트 누출 | **6.08 %** | 게이트 기준이 `self.q_cmd`였다 |

**수정:** 자체 `RLock`, `_commit()` **한 곳**에서 `T_cmd`/`q_cmd`/`_last_issued` 원자적 갱신,
`_hold()`는 `_last_issued`를 반환. `step()` 전체가 락 안에서 돈다
(`policy_delta_controller.py::PolicyDeltaController._commit` · `._hold` · `._step_locked`).

🛑 **이것이 G2를 닫지는 않는다.** 위 표의 기능 격차(branch-lock IK, `sigma_min` 감속,
keepout, anti-windup)는 **하나도 변하지 않았다.** 고쳐진 것은 "여러 스레드가 부를 때
상태가 찢어지는 것"뿐이고, 그 결함 1번은 **정확히 G2의 IK 분기 튐 위험을 증폭하던 경로**다.
...
> ### 🔧 2026-07-30 **오후** 갱신 (`edbb3f5`) — 증상이 바뀌었을 뿐, **근본 원인은 여전히 이 갭이다**
>
> **이전 판(보존):** *위 두 블록의 "개입 최고속이 12.5 → 6.3 → 1.8 cm/s로 떨어진다" 및
> "`BUDGET_EXHAUSTED`가 훨씬 빨리 온다"* — 이 서술은 `follow_mode="in_window"` 기준이고,
> 그 모드는 **더 이상 기본값이 아니다.**
>
> 오후에 추종이 데몬 스레드로 나가면서 **창 밖 공백이 사라졌다.** 그래서 이 갭이 조작자에게
> 나타나던 두 경로 중 하나는 닫혔다:
>
> | 경로 | 상태 |
> |---|---|
> | (a) 업샘플러 stale HOLD (51.5 %, 리더 추종 66 %) | 🟢 **닫혔다** — 추종 스레드가 33 ms마다 타깃을 내므로 `target_stale_s = 0.30`에 도달하지 않는다 |
> | (b) 예산 소진으로 최고속 하락 | 🟢 **닫혔다** — 개입 제어 경로에서 예산이 제거됐다 |
> | **(c) 기록 포화 (신규)** | 🔴 **열림.** 창이 길면 저장 액션이 실제 이동을 N배 **과소** 보고한다 |
>
> 🛑 **창 주기 자체는 하나도 안 변했다 — 실측 1.95 Hz / 512 ms(평균 0.502 s, 최악 0.854 s,
> learner 경합 1.12 s)다.** 예산을 뺀 뒤에는 그 길이가 **속도 제한이 아니라 기록 오염**으로
> 나타난다: `info["intervention_saturation"]`이 그 배수이고, 그 값을 1.0에 붙여 두는 유일한
> 코드적 해법이 **T를 줄이는 것**, 즉 이 갭이다. 다른 하나는 서버측 제외(→ **G33**)다.
> 전체 서술은 [`04_HIL_INTERVENTION.md`](04_HIL_INTERVENTION.md) **§10.2**.
...
## G24 — 개입 손맛: 타깃 갱신율이 낮아 **매 주기 가속/제동/정지** 🟡 **창 밖까지 해결 (2026-07-30 오후)**

조작자가 개입 중 팔이 **빳빳하고 덜덜거린다**고 보고했다. 원인은 필터도 게인도
가속도 제한도 아니라 **타깃 갱신 주기**다.

> ### ✅ 상태 (2026-07-30 **오후**, `edbb3f5` + `d6965a9`) — 추종이 **RL 창에서 분리됐다**
>
> 아래 오전 블록의 3 run은 **`follow_mode="in_window"`** 기준이고, 그 모드는 이제 기본값이
> 아니다. 오후에 `UR7eEnv`가 **데몬 추종 스레드**를 돌리도록 바뀌었다 — ENGAGED 동안 팔이
> 30 Hz로 리더를 계속 따라가고 `env.step`은 **관찰자**가 되어 transition만 뽑는다.
> **`InterventionBudget`이 개입 제어 경로에서 제거됐다**(조작자 결정). 남은 상한은
> **governor + 워크스페이스 박스 + 250 Hz 업샘플러**뿐이고 셋 다
> `PolicyDeltaController.step` 안에 있다. `follow_mode="in_window"`로 이전 경로를 보존한다.
>
> | | 이전 (in-window) | 이후 (background) |
> |---|---|---|
> | 개입 최고속 | `ACTION_SCALE / T` = **2.4 cm/s** (실측 T = 512 ms) | 예산 없음 — governor `v_max` **0.15 m/s**가 유일한 속도 상한 |
> | 창 밖 HOLD | **27.7 %** | 없음 |
> | slew 상한 | **46 %** | **100 %** |
> | 데드맨 release | **512 ms** | **33 ms** (추종 틱마다 재읽기) |
>
> 실측 근거: production actor 루프 **1.95 Hz**(대표 상수 512 ms; 실측 평균 0.502 s = 1.99 Hz,
> 최악 0.854 s, learner 경합 1.12 s). `env.step`은 **100 ms** 페이싱이고 나머지 **~412 ms**가
> gRPC `Step` RPC + 카메라 디코드로 **`env.step` 밖**에 있다.
>
> **조작자 확인 (2026-07-30 오후): "개입 속도는 좀 고쳐졌어".**
>
> 🛑 **그래도 🟢이 아니다 — background 경로의 체계적 실기 검증이 아직 없다.**
> 오전 3 run의 CSV 판정(frame-map 잔차 / `dp_ratio` / held-rate)을 이 모드로 돌린 적이 없다.
> 있는 것은 위 증언 하나와, `d6965a9`가 고친 **actor 즉사 1건**
> (`ActorProtocolError: executed_action must be within [-1, 1]`, `04` §10.3)뿐이다.
>
> **새로 생긴 대가와 그것이 만든 갭:** 예산이 없으므로 긴 창은 저장 액션이 실제 이동을
> **과소 보고**한다(측정치는 `info["intervention_saturation"]`). 근본 해결 둘 → **G33**
> (서버측 제외) · **G21**(창 주기 단축). 미배선 게이트 → **G32**. 러너 안전성 역전 → **G34**.
> 스레드 안전 수정 3건 → **G2**. 전체 서술은
> [`04_HIL_INTERVENTION.md`](04_HIL_INTERVENTION.md) **§10**.
>
> 📌 **테스트 기준선 (07-30 오후, `d6965a9`): `gello-hil-actor` 701 passed / 11 skipped /
> 1 xfailed, `hilserl` 741 / 4 / 1.** (07-30 오전 `4197f5b` 시점의 579·595 계보는 아래
> "코드 조치 상태" 표와 G25에 남겨 둔다 — **오전 수치와 섞지 말 것.**)

> ### ✅ 상태 (2026-07-30 **오전**) — 조치가 커밋되고 **실기에서 통과했다** *(in-window 경로)*
...
1. **나머지 절반은 여전히 G21 종속이다.** 두 갈래로 나타난다:
   ...
   > 🔧 **정정 (07-30 오후):** **이전 판(보존)** = 바로 위 (a)·(b) 두 문장. background 모드에서
   > **(a)와 (b)는 모두 닫혔다** — 추종 스레드가 33 ms마다 타깃을 내므로 stale HOLD에 도달하지
   > 않고, 예산 자체가 없다. 대신 같은 창 길이가 **기록 포화**로 나타난다(액션 1.0인데 실제로는
   > N배 이동) → **G33** · G21 오후 갱신 블록 · `04` §10.2. **창 주기(1.95 Hz)는 그대로다.**
...
6. **(07-30 오후 신규) background 경로가 만든 것 넷.** ① 운영자 WAIT 게이트에서 추종자가
   안 꺼진다 → **G32**, ② 포화 transition 서버측 제외 미구현 → **G33**, ③ `run_real_hil.py`로
   ARM하는 것이 production보다 덜 안전해졌다 → **G34** · G1, ④ 포화 계측의 **actor 요약
   배선이 없다** — 지금 그 숫자를 볼 수 있는 곳은 `run_real_hil.py` CSV뿐이다.
...
- 개입 경로는 지금 영향이 없다 — `_paced_request`가 요청을 `ACTION_SCALE/3 = 0.00417 m`로
  ...
  **즉 이 갭은 실질적으로 정책 경로의 문제다.**
  > 🔧 **정정 (2026-07-30 오후, `edbb3f5`).** 바로 위 문장은 **`follow_mode="in_window"`
  > 기준이다.** background 모드에는 `_paced_request`도 예산도 없고 `follow_xi`가 무제한
  > 델타를 내므로 **요청이 governor cap에 닿을 수 있다** — 즉 개입 경로에서도 대각 절삭이
  > 실제로 일어날 수 있고, 이제 `info["governed"]`가 그것을 **관측할 수 있다**(창 전체 OR /
  > `governed_scale` 최솟값). 🛑 **아직 실기에서 재지 않았다.** → G24 오후 블록, `04` §10.
...
`edbb3f5`가 `ur7e_env.py::UR7eEnv.suspend_follower`(컨텍스트 매니저)를 **만들었지만 아무도
부르지 않는다.** 그 docstring이 스스로 *"NOT WIRED UP YET"*이라고 적고 있고, 부를 곳이
어디인지도 이름으로 지목한다 — `ur_env/remote_actor.py`의 두 운영자 게이트다.

| 상태 | 지금 일어나는 일 |
|---|---|
| `WAIT_SCENE_READY` | RL 스레드를 **무한 블록**한다. 데드맨이 잡혀 있으면 추종 스레드는 **계속 팔을 몬다** |
| `WAIT_HOME_APPROVAL` | 같고, 🔴 **여기서는 데드맨을 아예 확인하지 않는다** |

즉 조작자가 프롬프트에 답하는 **동안 내내** 팔이 리더를 따라갈 수 있다. in-window 경로에서는
이 문제가 없었다 — 추종이 `env.step` 안에만 있었고 게이트는 `env.step` 밖이라 구조적으로
멈춰 있었기 때문이다. **창 밖으로 추종을 꺼내면서 같이 생긴 갭이다.**

### 왜 이번 커밋이 안 고쳤나

게이트가 `ur_env/remote_actor.py`에 있고 **그 파일은 다른 세션 소유**다. `edbb3f5`는
그것을 의도적으로 건드리지 않았다 — 이름 붙은 후속 작업으로 남긴다.

### 완화책 (지금 쓸 수 있는 것)

1. 🛑 **`WAIT_SCENE_READY` / `WAIT_HOME_APPROVAL` 화면에서는 GELLO 리더에서 손을 떼라.**
   데드맨을 놓으면 추종자는 **33 ms 안에** disarm된다(`_follow_tick`이 매 틱 재읽기).
   `04_HIL_INTERVENTION.md` **§10.4**에 조작자 문구로 있다.
2. 배선할 때의 계약: **재무장은 호출자 책임**이다. 승격은 오직 **RL 스레드의 스텝 경계**에서만
   허용되고 컨텍스트 매니저는 자기가 그 경계에 있는지 알 수 없다
   (`suspend_follower` docstring).

---

## G33 — 포화 transition의 **서버측 제외**가 미구현 🟠 (신규 2026-07-30 오후)

### 사실

개입 제어 경로에서 `InterventionBudget`이 제거되면서(→ G24 오후 블록, `04` §10.2),
**긴 창의 저장 액션이 실제 이동을 과소 보고**한다. 창 순변위를 `ACTION_SCALE`로 나눈 값의
norm이 1을 넘으면 비례 축소되어 저장되므로, transition은 *"액션 1.0이 1스텝만큼 움직였다"*고
말하지만 실제로는 **N스텝** 움직였다. critic은 동역학을 **낙관적으로** 학습한다.

이 방향은 `config.py`의 `ACTION_SCALE` INVARIANT 주석과 `serl_ur_infra/README.md`의
저장 액션 불변식이 **명시적으로 금지하는 방향**이다. 그것을 알고도 채택했다 —
2.4 cm/s로는 조작이 불가능하고 조작할 수 없는 개입 경로는 **시연을 0개** 만들기 때문이다.

**대가는 가정하지 않고 측정된다:** `info["intervention_saturation"]`(클립 **전** 비율) ·
`intervention_saturated` · `intervention_follow_ticks`, 그리고 `tests/run_real_hil.py`의
CSV 컬럼 `saturation` / `saturated` / `follow_ticks`와 `::summarize`의 포화 요약.
🛑 **wire로는 보내지 않는다 — 로컬 계측 전용이다.**

### 무엇이 미구현인가

포화 창을 **서버에서 학습 대상에서 빼는 것**이다. 필요한 것:

1. proto에 **신규 필드**(포화 플래그/비율),
2. **pb2 재생성**,
3. **`SCHEMA_VERSION` bump**,
4. **양 끝(actor·Kanu learner)을 같이 올리기.**

### 🛑 왜 반쪽 업그레이드가 특히 위험한가

**protobuf는 unknown field를 조용히 버린다.** actor만 올리고 Kanu를 안 올리면, 서버는
포화 플래그를 **못 본 채로** 과소 보고된 액션을 **아무 증상 없이** 학습한다. 예외도,
경고도, 스키마 불일치 에러도 없다. **무증상 오염**이고, 사후에 어떤 transition이 오염됐는지
복원할 수단도 없다(그 정보는 wire에 없다).

### 대안 경로

**창 주기 자체를 줄이는 것** → **G21**. 포화의 근본 원인은 창이 512 ms라는 것이다.
CSV의 포화 비율이 **둘 중 무엇을 먼저 할지의 근거**가 되도록 계측을 넣은 것이다.

### 완화책

1. **세션마다 포화 비율을 본다.** `run_real_hil.py --csv`로 남기고 `::summarize` 요약
   (포화 창 수 / 중앙값 / 최댓값)을 기록한다.
2. 이 경로로 만든 데이터를 offline 학습·데모 변환에 쓸 때 **"액션 1.0 = `ACTION_SCALE`"이
   아니라고 명시**한다. G26(대각 절삭, 과대 방향) · G27(line search, 과대) · G28(브레이크 창,
   과소)과 **네 번째** 오염 경로다.

---

## G34 — 예산이 빠진 지금 `run_real_hil.py`로 ARM하는 것이 **production보다 덜 안전하다** 🟠 (신규 2026-07-30 오후)

### 사실

`serl_ur_infra/tests/run_real_hil.py`는 `DefaultUR7eEnvConfig`를 쓰고, 그 config는
`ABS_POSE_LIMIT_*`가 0벡터라 **워크스페이스 박스가 꺼져 있다** → **G1**. 이것은 07-28·07-30
오전 실기에서도 그랬고 그때는 기재만 된 사실이었다.

**오후에 그 사실의 무게가 바뀌었다.** 개입 제어 경로에서 `InterventionBudget`이 제거되면서
`follow_xi`가 반환하는 델타를 **위치로** 묶는 것은 **워크스페이스 박스뿐**이다. governor는
**속도** 상한(`v_max` 0.15 m/s)이지 위치 상한이 아니다 — 코드 주석의 산술을 그대로 인용하면
**5초짜리 gRPC stall 동안 약 75 cm의 자유 이동**이 허용되고, 이는 측정된 박스 x 범위의
**2.8배**다(`wrappers.py::GelloIntervention.follow_xi`).

### 그래서 무엇이 뒤집혔나

**검증 순서다.** 지금까지 `run_real_hil.py`는 "actor보다 덜 위험한 러너"로 쓰였다(gRPC 없음,
정책 액션 zero). 그런데 박스가 켜진 `cube_in_cup` config로 도는 production actor 경로와 비교하면,
**이 러너가 유일한 위치 상한을 끄고 도는 쪽**이다.

| | `run_real_hil.py` | production actor (`cube_in_cup`) |
|---|---|---|
| 워크스페이스 박스 | ❌ **꺼짐** (`DefaultUR7eEnvConfig`) | ✅ 켜짐 |
| 예산 | ❌ 없음 (background 모드) | ❌ 없음 |
| 위치 상한 | **없음** | 박스 |

### 완화책

1. **`--arm`으로 이 러너를 돌릴 때는 조작자가 팔의 위치 상한이라고 가정하라.** 데드맨을
   놓으면 33 ms 안에 멈춘다는 것이 실질적인 유일한 보호다.
2. 근본 해결은 **박스가 켜진 config로 이 러너를 돌리는 것**이다 → **G1**. 그때까지
