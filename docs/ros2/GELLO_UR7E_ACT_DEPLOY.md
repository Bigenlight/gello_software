...
>
> **덫 2 — 조용한 실패 (이게 진짜 위험한 부분).** 해석되지 않는 시리얼로 바인딩해도
> **시끄럽게 실패하지 않는다.** 노드는 정상 기동하고 `ros2 topic info`는 publisher 1을
> 보고하는데, **프레임이 하나도 안 나온다.** 스택 어디에도 "카메라가 안 꽂혔다"와
> "설정이 틀렸다"를 구분해 주는 신호가 없다. 그래서 아래 절차는 시리얼 하드코딩보다
...
> 아래 §8의 "실기 검증" 기록이 `147122072740` / `243222072700`을 인용하는 것은 **처음부터
> 옳았다**. 07-28 판이 그 줄에 "옛 개체라 오늘 쓰면 안 된다"는 주석을 달았던 것은 잘못된
> 정정이었고, 철회했다.
...
3. **RealSense 카메라 2대를 정확한 시리얼→네임스페이스 매핑으로 기동.**

   **권장 — 래퍼를 쓸 것.** [`ros2_ur_ws/launch_cameras.sh`](../../ros2_ur_ws/launch_cameras.sh)는
   `_resolve_camera_serials.sh`를 source해 **실제 USB 버스에 대고 시리얼을 해석**하고(설정된 쌍이
   안 꽂혀 있으면 모델 클래스로 재배정 + WARN), 두 컬러 스트림이 ~30 Hz로 실제로 흐를 때까지
   기다린 뒤 cam1/cam2 뷰어를 띄운다. 시리얼을 손으로 적지 않는 것이 요점이다 —
   **틀린 시리얼은 에러가 아니라 무음으로 나타나기 때문**이다(§2 정정 박스, §9):

   ```bash
   cd ~/gello_software/ros2_ur_ws
   ./launch_cameras.sh              # 카메라 2대 + 라이브 뷰어 (Ctrl-C 한 번으로 정리)
   # VIEW=false ./launch_cameras.sh # 뷰어 없이 카메라만
   ```

   **수동(fallback).** 굳이 손으로 띄운다면 `run_recorder.sh`가 쓰는 것과 동일한 형식
   (시리얼별 `realsense2_camera` 노드, `serial_no:='<serial>'`처럼 **작은따옴표로 감싼**
   all-digit 문자열, 공통 `color_profile:='1280x720x30'`)이다. 값은 **장치 시리얼**이어야 한다:
   ...
   기동 후 압축 이미지 토픽이 실제로 나오는지 확인 (수동 경로에서는 **이 확인이 필수**다 —
   노드가 떠 있다는 것만으로는 프레임이 나온다는 보장이 전혀 없다):
...
cd ~/gello_software/ros2_ur_ws
./launch_cameras.sh          # 권장: 시리얼 자동 해석 + 스트림 대기 + 뷰어(실행 제어 버튼 포함)
```

```bash
# fallback — 손으로 띄울 때. 값은 반드시 **장치 시리얼**(ASIC 시리얼 아님, §2 정정 박스).
source /opt/ros/humble/setup.bash
cd ~/gello_software/ros2_ur_ws
ros2 launch realsense2_camera rs_launch.py \
    camera_name:=cam1 camera_namespace:=cam1 \
...
**`ros2 topic info`의 publisher 수는 확인이 되지 않는다** — 시리얼이 해석되지 않아도 1로 나온다.
반드시 `topic hz`로 프레임이 실제로 흐르는지 볼 것.
...
- **실제 카메라 시리얼 매핑** — cam1(D435, `147122072740`)·cam2(D435if, `243222072700`) 모두 `rs-enumerate-devices`로 정상 열거, arm 직전 `ros2 topic hz`로 압축 토픽이 ~29.7–29.9Hz 라이브 스트리밍 확인 — 문서의 예상 물리 배치와 일치.
  > ✅ **이 줄은 처음부터 옳았고, 지금도 유효하다.** 2026-07-28에 누군가 여기에 "이 시리얼은
  > 당시 장착돼 있던 옛 개체이고 카메라가 물리적으로 교체됐으니 오늘 이 값으로 실행하면
  > 조용히 안 뜬다"는 경고를 달았는데, **그 정정이 틀렸다.** 카메라는 교체된 적이 없고
  > `147122072740` / `243222072700`은 지금도 이 두 대의 **장치 시리얼**이다(§2 정정 박스).
  > 07-28 경고는 철회한다.
...
- **카메라가 조용히 안 뜬다 (노드는 살아 있는데 프레임이 0).** 십중팔구 **해석되지 않는 시리얼**을 바인딩한 것이다. `journalctl`/`/sys/bus/usb/devices/*/serial`이 보여주는 값은 **ASIC 시리얼**(`151623020789` / `322743060038`)이고 `serial_no:=`는 **장치 시리얼**(`147122072740` / `243222072700`)에만 매칭된다. 이 실패는 에러를 내지 않고 `ros2 topic info`도 publisher 1을 보고한다 — `ros2 topic hz`로만 구분된다. **저널 grep으로 카메라 정체를 재유도하지 말 것**(그 방법으로 반대 방향의 틀린 결론에 도달한 세션이 둘 있다). 실제 값은 `rs-enumerate-devices`로 보거나, 그냥 [`launch_cameras.sh`](../../ros2_ur_ws/launch_cameras.sh)에 맡길 것.
