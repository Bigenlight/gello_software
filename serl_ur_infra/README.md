...
| [KANU_FALLBACK_TEST_KO.md](KANU_FALLBACK_TEST_KO.md) 🆕 | 🛑 **브랜치 `test/kanu-learner-fallback` 전용.** junhyeong_ai GPU가 무기한 점유돼 learner만 옛 서버 `kanu`(GPU 1)로 임시 대체하는 절차 — kanu에 준비한 것은 **classifier 사본 하나**(`~/hil-serl-data/classifier_ckpt/checkpoint_150`, SHA 검증), 터미널 3개 명령(**T1 `run_hil_server_kanu.sh`**, T3는 `HIL_SSH_HOST=kanu` + `HIL_SERVER_SCRIPT` **둘 다** 필요), dry run 실측(READY까지 ~2.5분 · learner step ≈439 ms · params staleness ~22–25 s **무해** · 백로그는 더 커진다), GPU 공유 예의, 되돌리기. **main에 올라간 것은 ssh `ControlPath` 수정(`15a467e`) 하나뿐**이고 `junhyeong_ai`는 그 커밋으로 checkout만 pull됐다(데이터·프로세스 무변경). 🛑 **실기 미검증 — dry run만.** ⚠️ 이 브랜치에 서서 junhyeong_ai를 부르면 HEAD 불일치 배너가 뜬다(§5 함정 6) — 정상 서버로 돌아갈 때는 **main으로 checkout부터** |
