...
### 🔴 "무크롭"은 지금의 실제 추론 경로와 다르다

`test/hil-hardware-comms` 머지(`3f199d4`)가 `serl_ur_infra/ur_experiments/cube_in_cup.py`를 들여왔고,
그 `IMAGE_CROP`이 cam1 `img[20:670, 340:990]`(650×650) / cam2 `img[0:720, 420:1140]`(720×720)로 채워져 있다.
gRPC 경로(port 50053)의 canonical observation은 이제 **크롭된** 입력이다.

**측정이 틀린 것이 아니다. 조건이 붙는 것이다.** 이 문서의 표는 **무크롭 입력에 대해 지금도 유효**하고,
**크롭이 활성인 현재 gRPC 경로에는 적용되지 않는다.** 실측 근거: 픽셀 MAE 0.00(무크롭 가설과 비트 일치)
대 21–35(크롭), recall@0.85 **100.0% → 33.3%**.

**해결 방향은 크롭 제거가 아니다.** 크롭 값은 데이터셋에서 실측해 나온 값이고 1차 소비자는 RL 정책이다.
정본 경로는 **같은 크롭으로 classifier를 재학습하고 이 문서의 스윕을 다시 돌리는 것**이다
(→ [후속 권고 (d)](#후속-권고)). **즉 이 문서의 스윕은 폐기 대상이 아니라 재실행 대상이다.**
자세한 것은 [전처리 절](#전처리--이-문서의-수치는-무크롭-조건부다).
...
### 전처리 — 이 문서의 수치는 무크롭 조건부다

**현재 상태(2026-07-29, 머지 `3f199d4` 이후): 학습은 무크롭, 추론은 크롭. 불일치가 실재한다.**

| | 전처리 |
| --- | --- |
| classifier 학습 + 이 문서의 모든 채점 | 1280×720 전체 → 128×128 (**무크롭**, `preprocess_frame(frame, None)`) |
| gRPC 경로의 canonical observation (`ur7e_env.get_im()`) | `IMAGE_CROP` 적용 후 128×128 (**cam1 650×650, cam2 720×720**) |
| ZMQ 뷰어(`decode_classifier_image()`) | 무크롭 — **그 용도에서는 올바르다. 크롭 불일치의 원인이 아니다** |
...
**측정은 무효가 아니다. 조건이 붙는 것이다.** 이 문서의 표는 **무크롭 입력에 대해 유효**하고,
**크롭이 활성인 경로에는 적용되지 않는다.** 실측: 픽셀 MAE 0.00(무크롭 가설과 비트 일치) 대 21–35(크롭),
recall@0.85 **100.0% → 33.3%**.
...
**(d) 크롭 정합 classifier를 재학습하고 이 문서의 스윕을 통째로 다시 돌려라.** 🔴 최우선
...
5. 전환 전에 `checkpoint_sha256()`의 `os.path.isfile` 제약(orbax 디렉터리 pin 불가)을 먼저 풀어야 한다
   ([구 checkpoint 절](#구-checkpoint는-사용-불가)).
