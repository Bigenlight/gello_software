# reward classifier threshold 결정 기록 — 현재 0.5, 0.2는 과거 결정
...
> - threshold는 learner fingerprint에 포함되므로 `0.2` lineage를 `0.5` run에 resume하지 않는다.
...
`0.85` → `0.5`(`53d5cf6`) → `0.2`(`1b02857`) → **`0.5`(2026-07-30 production)**.
이 문서의 과거 결정값이 코드 기본값이라고 **가정하지 마라**.
권위는 이 문서가 아니라 `ur_env/rlpd_receive_server.py:73`의 `DEFAULT_REWARD_THRESHOLD`에 있다
(2026-07-30 확인 시점 값 `0.5`). 인용 전에 그 줄을 직접 열어 확인하라.

## ✅ 당시 결정 (2026-07-29): **0.2** — 현재 0.5로 대체됨
...
4. **0.05는 여전히 금지다.** 이미 관측된 negative(`0.1428`, `fold_take_02` / 0720 val / 무크롭 / 07-29)
   아래다.
5. **이 결정은 무크롭 측정 위에 서 있다.** 위 recall/FPR은 전부 크롭 없는 입력에서 잰 값이다.
   현재 정책 관측은 크롭을 쓰지만 classifier sidecar는 전용 무크롭 이미지를 받아 이 측정 조건과
   일치한다. 향후 classifier checkpoint나 전처리를 바꾸면 같은 스윕을 다시 실행하고 learner
   fingerprint도 함께 바꿔야 한다.

> 이 절은 2026-07-29 당시의 결정이고, 이하 본문은 결정에 이르기까지의 측정 기록이다. 본문의
> "보류" 표현은 07-29 오전 시점의 판단이다. **현재 실행에는 문서 최상단의 2026-07-30 계약이 우선한다.**

## 🔴 이 문서는 2026-07-29 감사로 핵심 수치가 교정됐다

**이 감사 시점의 결론은 threshold 0.5였다**(그 뒤 07-29 오후에 비용 비대칭을 근거로
**0.2**로 다시 내려갔다 — [당시 결정 절](#-당시-결정-2026-07-29-02--현재-05로-대체됨)).
...
**threshold는 learner fingerprint에 들어간다.** 따라서 다른 threshold로 학습한 checkpoint는 현재
`0.5` run에서 resume할 수 없다. 기본값은 `0.85 → 0.5`(`53d5cf6`) → `0.2`(`1b02857`) →
`0.5`(2026-07-30 production)로 움직였고, 이동할 때마다 별도 lineage가 된다.
...
사라진다. 07-29 당시 존재하던 checkpoint는 synthetic acceptance용뿐이어서 실질 피해가 없었다.
현재 production run이 시작된 뒤에는 threshold를 바꾸지 마라.

(이 절의 경로 설명은 07-29 감사에서 변경 없고, 07-30에는 기본값이 다시 0.5가 된 이력만
추가했다. 줄 번호는 코드 이동에 따라 달라질 수 있으므로 기호 이름으로 확인한다.)
...
*(그날 오후 비용 비대칭 판단으로 당시 값은 **0.2**가 됐다. 현재는 최상단 계약대로 `0.5`다.
아래 근거 자체는 두 결정의 이력 설명에 그대로 쓰인다.)*
...
> **판정 열의 역사적 상태**: `0.2 = 보류`는 **07-29 오전**의 판정이고, 같은 날 오후에 비용 비대칭을
> 근거로 채택됐다. 07-30 production 값은 다시 `0.5`이며, 마진 수치 자체는 그대로다.
...
> **🔴 [07-29 오후 당시 결정]** 위 "0.5를 유지하라"는 **margin을 기준으로 삼았을 때의**
> 결론이다. 운영자는 당시 기준을 **비용 비대칭**으로 바꿔 **0.2를 채택했다**. 이후 07-30에는
> MANUAL 기본 운용과 함께 production threshold를 `0.5`로 되돌렸다. 위 (a)(b) 조건은 classifier를
> 다시 AUTO 기본으로 승격할 때의 재검증 항목으로 남는다.
