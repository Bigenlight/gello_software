...
그 여유가 1.4배(최악 ckpt) / 2.33배(배포 ckpt)로 줄어드는 것을 알고 받아들인 결정이다.

### 이 결정에 붙는 조건

1. **첫 실험은 감시하에 돌린다.** 오탐이 한 번이라도 보이면 즉시 0.5로 되돌린다.
2. **새 footage가 들어오면 재측정한다.** 마진 논증 전체가 프레임 **한 장**
   (`take_11_20260720_205805` frame 195, 큐브 투하 약 0.2초 전)에 걸려 있다. 6개 checkpoint가
   전부 같은 프레임에서 최대값을 낸다. 이 경계 구간 데이터가 보강되면 숫자가 바뀔 수 있다.
3. **production run이 시작된 뒤에는 바꾸지 마라.** 아래 fingerprint 절 참조.
4. **0.05는 여전히 금지다.** 이미 관측된 negative(0.1428) 아래다.
