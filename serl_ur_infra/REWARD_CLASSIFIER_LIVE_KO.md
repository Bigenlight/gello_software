...
| 크롭 적용 시 실기 성능 | ❌ 미측정 (오프라인 채점에서만 100% → 33%) | [§3](#3--크롭-불일치--뷰어가-보는-것은-rl-loop가-보는-것이-아니다) |
...
## 3. 🔴 크롭 불일치 — 뷰어가 보는 것은 RL loop가 보는 것이 아니다

**이 절을 건너뛰면 이 문서를 읽을 이유가 없다.**

| | **이 뷰어 (로컬/ZMQ 경로)** | **gRPC RL 경로 (port 50053)** |
| --- | --- | --- |
| 입력 | 카메라 토픽 직접 구독 | canonical observation |
| 전처리 | `decode_classifier_image()` — **크롭 없음**, 1280×720 전체를 128×128로 리사이즈 | `ur7e_env.get_im()` — **`IMAGE_CROP` 적용 후** 128×128 |
| 학습 전처리와 일치? | ✅ **일치한다** | ❌ **불일치한다** |
| 로봇 | 안 움직인다 | 움직인다 |
...
> 🪤 **읽는 사람이 놓치면 안 되는 결론:**
> **뷰어에서 `SUCCESS`가 떠도, 같은 순간 gRPC 경로의 classifier는 실패로 판정할 수 있다.**
> 뷰어 관찰을 "learner가 reward를 잘 준다"의 근거로 쓰지 마라. 두 경로는 서로를 호출하지
> 않고, 지금은 전처리도 체크포인트 pin도 다르다.
>
> **해결 방향은 크롭을 지우는 것이 아니라 크롭 조건에서 재학습하는 것이다.**
> `IMAGE_CROP` 박스는 실측으로 정한 값이고(cam2는 그리퍼 파지축 x=781 중심),
> 지우면 크롭이 해결하려던 문제가 되돌아온다. `cube_in_cup.py:181-191`의
> `KNOWN CONFLICT (G15)` 주석이 같은 말을 코드 옆에 적어 두고 있다.
...
> ℹ️ 링크 수치는 시점 의존이다. [HANDOFF_NEXT_SESSION_KO.md](./HANDOFF_NEXT_SESSION_KO.md) §0은
> 같은 링크를 다른 조건(WiFi)에서 **약 13 Mbit/s ≈ 1.6 MiB/s**로 기록한다. 그 값이면 전송만
> 약 60 ms다. **어느 쪽 수치를 써도 결론은 같다.**
...
저장소에 적힌 기본 시리얼(`151623020789` / `322743060038`)이 이 PC에 꽂힌 것과 다를 수 있다.
`launch_cameras.sh`는 기동 전에 **살아 있는 USB 버스와 대조해 시리얼을 재배정한다**
(공용 헬퍼 `ros2_ur_ws/_resolve_camera_serials.sh`, 커밋 `43ba314`·`fb48100`).
