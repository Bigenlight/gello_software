#!/usr/bin/env python3
"""Generate `dataset_stats.json` for a RAW take-folder release (task-agnostic).

This is the task-agnostic sibling of `make_carrot_raw_stats.py` (which stays
untouched — other docs reference it by name/line and its own release is frozen).
Everything here is identical EXCEPT: `--data` / `--dataset` / `--task` are required
CLI args instead of carrot-specific defaults, the output JSON carries a top-level
`"task"` string, and a `stamp_s` block is added (see below) for corpora recorded
with the recorder-timestamp-lag fix (header `stamp_s` stored per row instead of
only callback-time `t_rel_s`).

Reads every `take_*/vectors.h5`, optionally `take_*/depth.h5`, `take_*/cam1.mp4` and
`take_*/cam2.mp4` in the staging directory and writes one machine-readable JSON with
every number a README / DATA_DICTIONARY for that release would want to quote.
...
writes is `<data-dir>/dataset_stats.json` (or `--out`).
...
Usage:
    python3 make_raw_stats.py \
        --data /home/laptop3/youngwoong_ws/Orange_bowl_in_purple_bowl \
        --dataset Bigenlight/orange_bowl_in_purple_bowl_raw \
        --task "put the orange bowl into the purple bowl" \
        [--out <path>] [--no-depth | --depth]

Definitions (kept identical to sibling raw releases so the JSONs are comparable):
  ...
  * the `timestamp_lag` block measures the (pre-fix) recorder timestamp artefact:
    how many seconds late `ur_joint_states` / `tcp_pose` / `wrench` rows are
    stamped, per take, by joint-space residual and by speed cross-correlation. It
    is left in unconditionally because a near-zero result IS the evidence that a
    corpus was recorded with the fixed recorder (see the `stamp_s` block below,
    which carries the same evidence from the ROS header stamp directly).
  * the `stamp_s` block (only meaningful for corpora whose vectors.h5 groups carry
    a `stamp_s` column: cam1_frames, cam2_frames, gello_joint_states, tcp_pose,
    ur_joint_states, wrench) reports, per table: the pooled median/p99 inter-sample
    interval measured on the ROS header clock, the pooled-across-takes median rate
    computed from `t_rel_s` vs. from `stamp_s`, and (cheap via the same
    nearest-timestamp search used elsewhere in this script) the median offset of
    that table's `stamp_s` against `cam1_frames`' `stamp_s` at the same `t_rel_s`.
"""
...
# Groups that carry a `stamp_s` column (ROS header stamp, absolute epoch seconds) in
# the fixed recorder. See the `stamp_s` block in the output.
STAMP_TABLES = (
    "cam1_frames",
    "cam2_frames",
    "gello_joint_states",
...
# fingers met each other, not the grasped object). Measured empty-close plateau is
# 0.898 (carrot_in_pot_raw measurement; the discretization thresholds are shared
# across releases per project convention — do not re-derive per task).
...
def main():
    ...
    ap.add_argument("--dataset", required=True, help="HF dataset id, e.g. Bigenlight/foo_raw")
    ap.add_argument("--task", required=True,
                    help="language task string, written into the JSON as \"task\", e.g. "
                         "'put the orange bowl into the purple bowl'")
    ...
    # stamp_s support: auto-detect from the first take. All takes in one staging
    # directory come from the same recorder version, so this is uniform across a
    # release; a corpus recorded before the recorder-timestamp-lag fix simply has
    # no `stamp_s` column and the whole block degrades to `"present": False`.
    with h5py.File(os.path.join(data_dir, takes[0], "vectors.h5"), "r") as f0:
        have_stamp_s = all("stamp_s" in f0[g] for g in STAMP_TABLES if g in f0)
    if not have_stamp_s:
        print("no stamp_s column found (pre-fix recorder) - stamp_s block will "
              "record presence=False only", file=sys.stderr)
    ...
    stamp_dt_pool = {g: [] for g in STAMP_TABLES}
    stamp_rate_trel_pool = {g: [] for g in STAMP_TABLES}
    stamp_rate_stamp_pool = {g: [] for g in STAMP_TABLES}
    stamp_rate_ratio_pool = {g: [] for g in STAMP_TABLES}
    stamp_cam1_offset_pool = {g: [] for g in STAMP_TABLES if g != "cam1_frames"}
    ...
    for ti, take in enumerate(takes, 1):
        ...
        with h5py.File(vec_path, "r") as f:
            ...
            for g in VECTOR_GROUPS:
                ...
                if have_stamp_s and g in STAMP_TABLES:
                    stamp_data[g] = {"t_rel_s": t, "stamp_s": grp["stamp_s"][:]}
            ...
            # stamp_s block (ROS header clock) ---------------------------------
            # Cheap because it reuses the same nearest-timestamp search as the
            # leader/follower alignment below; skipped entirely (have_stamp_s=False)
            # for corpora whose recorder predates the stamp_s column.
            if have_stamp_s:
                cam1_t = stamp_data.get("cam1_frames", {}).get("t_rel_s")
                cam1_s = stamp_data.get("cam1_frames", {}).get("stamp_s")
                for sg in STAMP_TABLES:
                    if sg not in stamp_data:
                        continue
                    gt_, gs_ = stamp_data[sg]["t_rel_s"], stamp_data[sg]["stamp_s"]
                    if gs_.size > 1:
                        stamp_dt_pool[sg].append(np.diff(gs_))
                    r_trel = mean_rate(gt_)
                    r_stamp = mean_rate(gs_)
                    if r_trel is not None:
                        stamp_rate_trel_pool[sg].append(r_trel)
                    if r_stamp is not None:
                        stamp_rate_stamp_pool[sg].append(r_stamp)
                    if r_trel is not None and r_stamp:
                        stamp_rate_ratio_pool[sg].append(r_trel / r_stamp)
                    if (sg != "cam1_frames" and cam1_t is not None and cam1_t.size > 1
                            and gt_.size):
                        idx = nearest_idx(cam1_t, gt_)
                        offset = gs_ - cam1_s[idx]
                        stamp_cam1_offset_pool[sg].append(float(np.median(offset)))
    ...
    timestamp_lag = {
        "note": (
            "What this block measures: how far `ur_joint_states` (and, by cross-"
            "correlation, `tcp_pose`) trail `command` on the recorded t_rel_s timebase. "
            "HOW TO READ IT depends on which recorder wrote the corpus. (a) PRE-FIX GUI "
            "recorder (no `stamp_s` columns; e.g. Bigenlight/carrot_in_pot_raw, "
            "2026-09-14): t_rel_s is the time the subscription callback RAN, all "
            "callbacks shared one spin thread, and depth writes starved it, so "
            "/joint_states (KEEP_LAST 100 at ~100 Hz) left the queue ~0.9-1.0 s old and "
            "tcp_pose/wrench (depth 50) ~0.5 s old - a RECORDER ARTEFACT with the "
            "starvation signature that command/ur_joint_states/tcp_pose/wrench all "
            "collapse onto one recorded rate; the fix rule for consumers was to "
            "subtract the lag from those tables' t_rel_s. (b) FIXED recorder (header "
            "stamps stored as `stamp_s`, queue depth 5, frame I/O on a writer thread; "
            "commit 9d54490, 2026-09-14): nothing is stamped late, the four robot tables "
            "keep their own native rates, and whatever lag is measured here is the real "
            "arm's servo tracking delay behind the streamed joint target - genuine robot "
            "behaviour that is IN the data and must NOT be subtracted. "
            "`recorder_artefact_expected` below says which case this corpus is, from "
            "the presence of `stamp_s`; the `stamp_s` block (when present) carries the "
            "direct cross-table evidence. NOTHING IS REWRITTEN HERE: the raw files are "
            "as recorded."
        ),
        "recorder_artefact_expected": (not have_stamp_s),
        "method": (
    ...
    if have_stamp_s:
        stamp_s_block = {
            "present": True,
            "note": (
                "ROS header stamp (absolute epoch seconds), stored per-row by the FIXED "
                "recorder alongside the callback-time `t_rel_s`. A near-zero "
                "stamp_minus_cam1_stamp offset and a rate_ratio_trel_over_stamp near "
                "1.0 for every table is the evidence that this corpus does NOT carry "
                "the recorder-timestamp-lag artefact described in `timestamp_lag` "
                "above (contrast a pre-fix corpus, where ur_joint_states/tcp_pose/"
                "wrench are stamped ~0.5-0.9 s late on t_rel_s but stamp_s is exact)."
            ),
            "tables": list(STAMP_TABLES),
            "per_table": {
                g: {
                    "dt_stamp_s_median_ms": (
                        _r(float(np.median(np.concatenate(stamp_dt_pool[g]) * 1000.0)), 3)
                        if stamp_dt_pool[g] else None),
                    "dt_stamp_s_p99_ms": (
                        _r(float(np.percentile(np.concatenate(stamp_dt_pool[g]) * 1000.0, 99)), 3)
                        if stamp_dt_pool[g] else None),
                    "rate_trel_hz_median_over_takes": (
                        _r(float(np.median(stamp_rate_trel_pool[g])), 3)
                        if stamp_rate_trel_pool[g] else None),
                    "rate_stamp_hz_median_over_takes": (
                        _r(float(np.median(stamp_rate_stamp_pool[g])), 3)
                        if stamp_rate_stamp_pool[g] else None),
                    "rate_ratio_trel_over_stamp_median_over_takes": (
                        _r(float(np.median(stamp_rate_ratio_pool[g])), 4)
                        if stamp_rate_ratio_pool[g] else None),
                    "stamp_minus_cam1_stamp_at_same_t_rel_s_median_over_takes": (
                        0.0 if g == "cam1_frames" else
                        (_r(float(np.median(stamp_cam1_offset_pool[g])), 4)
                         if stamp_cam1_offset_pool.get(g) else None)),
                    "n_takes": len(stamp_rate_trel_pool[g]),
                }
                for g in STAMP_TABLES
            },
        }
    else:
        stamp_s_block = {
            "present": False,
            "note": "no `stamp_s` column found in this corpus's vectors.h5 groups "
                    "(pre-fix recorder); only t_rel_s-based rates are available.",
        }
    ...
    doc = {
        ...
        "generated_by": (
            f"make_raw_stats.py — h5py + cv2 + ffprobe audit over all {len(per_take)} "
