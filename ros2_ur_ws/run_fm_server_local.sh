#!/usr/bin/env bash
# =============================================================================
# run_fm_server_local.sh -- serve the FM policy from laptop3's OWN GPU
# =============================================================================
#
# Terminal 1 of a LOCAL flow-matching (FM) evaluation:  ./run_fm_server_local.sh
#
# Sibling of run_fm_server.sh with the network removed.  That script starts the
# FM server on junhyeong_ai and holds an SSH tunnel so the actor's fixed dial
# address 127.0.0.1:50153 reaches it; this one runs the SAME UNMODIFIED
# serl_ur_infra/scripts/run_fm_policy_server.py here and binds 127.0.0.1:50153
...
#     more likely to be a live run_hil_server.sh / run_fm_server.sh tunnel than
...
# possible -- do not "helpfully" preallocate here.  FM compiles its sampler at
# startup and that compile is the memory peak of the whole run.  Numbers
# measured on an idle card are not the numbers a live session sees; record
# which it was.
#
...
#   FM_ARTIFACT_DIR=<data root>/diagnostics/<fm-init artifact>
#   FM_WHICH=best (which trained parameter set inside the artifact to serve)
...
FM_ARTIFACT_DIR="${FM_ARTIFACT_DIR:-$LOCAL_DATA_ROOT/diagnostics/jax_fm_cube_in_cup_raw_0731_h16_euler8_200epoch_20260731_215835.fm-init}"
FM_WHICH="${FM_WHICH:-best}"
LOCAL_POLICY_PORT="${LOCAL_POLICY_PORT:-50153}"
FM_START_TIMEOUT_S="${FM_START_TIMEOUT_S:-180}"

# The pins an FM eval's actor terminal must carry.  Printed, never exported: T3
...
FM_EXPECTED_MODEL_ID="fm-cube-in-cup-raw0731-h16-euler8-v1"
EXPECTED_REWARD_AUTHORITY="local"
EXPECTED_REWARD_MODEL_ID="operator-manual-success-v1"

SERVER_ENTRYPOINT="$REPO_ROOT/serl_ur_infra/scripts/run_fm_policy_server.py"
READY_MARK='[fm-server] ready '
FATAL_MARK='[fm-server] FATAL'
STOPPED_MARK='[fm-server] stopped '

RUN_DIR="$LOCAL_DATA_ROOT/fm_eval/fm_eval_local_$(date -u +%Y%m%d_%H%M%S)"
...
# local end of run_hil_server.sh's tunnel.  Binding it would put a frozen FM
# policy behind the address an online-RL session dials.
(( LOCAL_POLICY_PORT != 50053 )) || \
    die "port 50053 belongs to the production learner (and its laptop3 tunnel); run_fm_server_local.sh will not use it"
...
# One bare token.  The vocabulary (best/final) is the server's to police, not
# ours -- it rejects anything else by argparse choices, with a clear message.
[[ "$FM_WHICH" =~ ^[A-Za-z0-9_-]+$ ]] || die "FM_WHICH must be a single bare word (got: $FM_WHICH)"
for _name in LOCAL_POLICY_PYTHON LOCAL_DATA_ROOT FM_ARTIFACT_DIR; do
    _value="${!_name}"
...
[[ -f "$SERVER_ENTRYPOINT" ]] || die "FM entrypoint is missing: $SERVER_ENTRYPOINT"
# No PYTHONPATH export, unlike the remote sibling: run_fm_policy_server.py
    ...
    cpu)
        {
            ...
            echo "         number this eval produces is a CPU number -- and FM's sampler"
            echo "         jit is the slowest thing in this stack on a CPU.  The ready"
            echo "         banner below will say so; do not file these as GPU numbers."
...
if [[ ! -d "$FM_ARTIFACT_DIR" ]]; then
    {
        echo "ERROR: fm-init artifact directory is missing:"
        echo "         $FM_ARTIFACT_DIR"
        echo "  Fetch it from the GPU server:   $SCRIPT_DIR/fetch_policy_artifacts.sh"
        echo "  Nothing was started."
    } >&2
    exit 1
fi
[[ -f "$FM_ARTIFACT_DIR/manifest.json" ]] || \
    die "no manifest.json in $FM_ARTIFACT_DIR -- the artifact looks incomplete; re-run $SCRIPT_DIR/fetch_policy_artifacts.sh"
...
if port_is_bound; then
    {
        ...
        if [[ -n "$_busy_pids" && "$_busy_pids" != "NOTOOL" ]]; then
            while read -r _busy_pid; do
                ...
                _busy_classified=1
                case "$_busy_cmd" in
                    *run_fm_policy_server.py*|*run_bc_policy_server.py*)
                    ...
                    *ssh*)
                        ...
                        echo "    (./run_hil_server.sh, ./run_fm_server.sh or ./run_bc_server.sh)."
        ...
        if (( _busy_classified == 0 )); then
            ...
            echo "  SSH tunnel from ./run_hil_server.sh / ./run_fm_server.sh /"
            echo "  ./run_bc_server.sh -- Ctrl-C that terminal -- or an orphaned local"
...
if (( avail_g < LOCAL_POLICY_MIN_FREE_G )); then
    die "only ${avail_g}G free on $LOCAL_DATA_ROOT -- need >= ${LOCAL_POLICY_MIN_FREE_G}G for an FM eval's served records.
  Free space by removing old local eval run dirs (they are pure output, nothing else reads them):
      du -sh $LOCAL_DATA_ROOT/fm_eval/fm_eval_local_* | sort -h
      rm -rf $LOCAL_DATA_ROOT/fm_eval/fm_eval_local_<timestamp>
...
{
    printf '[fm-local] launcher pid=%s python=%s device=%s\n' \
        "$$" "$LOCAL_POLICY_PYTHON" "$LOCAL_POLICY_DEVICE"
    printf '[fm-local] artifact=%s which=%s port=%s run_dir=%s\n' \
        "$FM_ARTIFACT_DIR" "$FM_WHICH" "$LOCAL_POLICY_PORT" "$RUN_DIR"
} >>"$LOGFILE"

echo "Starting FM policy server on laptop3 (port $LOCAL_POLICY_PORT)"
...
if (( step_timing_on )); then
    echo "[fm-local] step timing ON -> $RUN_DIR/served/timing.jsonl"
fi
...
stop_server() {
    ...
    # SIGINT, not SIGTERM: run_fm_policy_server.py installs a handler for both,
    ...
    # "[fm-server] stopped replay_count=... " summary, which is the record of
    ...
    # (after the jax import, the artifact load and the sampler jit: longer here
    # than for BC) this child is DEAF to SIGINT.  Measured: a stub that died at
    # startup made the launcher signal for the full 20 s before escalating.
    # After ready the handler is provably installed, because ready is printed
    # after it.
    echo "Stopping local FM server PID $SERVER_PID (INT)..."
    ...
    if server_running; then
        echo "FM server PID $SERVER_PID did not exit on INT after $(( grace / 2 ))s; sending TERM." >&2
    ...
    if server_running; then
        echo "FM server PID $SERVER_PID ignored TERM; sending KILL." >&2
...
# Expect tens of seconds before it appears, much longer than BC's: this server
# jit-compiles the flow sampler and smokes BOTH sampling modes (deterministic
# and stochastic) before it will call itself ready, precisely so the first real
# request of either mode cannot be the one that discovers a broken graph.
READY_LINE=""
DEADLINE=$(( $(date +%s) + FM_START_TIMEOUT_S ))
while :; do
    HIT="$(grep -F -m1 -e "$FATAL_MARK" -e "$READY_MARK" "$LOGFILE" 2>/dev/null || true)"
    if [[ "$HIT" == *"$FATAL_MARK"* ]]; then
        fail_with_log "FM server reported $HIT"
    fi
    if [[ -n "$HIT" ]]; then
        READY_LINE="$HIT"
        READY_SEEN=1
    ...
    server_running || \
        fail_with_log "FM server process exited before reporting ready"
    (( $(date +%s) < DEADLINE )) || \
...
while read -r _pid; do
    ...
    fail_with_log "port $LOCAL_POLICY_PORT is held by PID $_pid (${_cmd:-<unreadable>}), which is NOT the FM server we started (PID $SERVER_PID). If that is an ssh tunnel, this run would have measured the REMOTE server while every pin still passed. Close the remote server terminal and start again."
done <<<"$LISTENER_PIDS"
...
echo "============================================================"
echo "FM policy server ready -- $DEVICE_BANNER"
...
echo "  EXPECTED_MODEL_ID=$FM_EXPECTED_MODEL_ID"
...
echo "  EXPECTED_MODEL_ID=$FM_EXPECTED_MODEL_ID EXPECTED_REWARD_AUTHORITY=$EXPECTED_REWARD_AUTHORITY EXPECTED_REWARD_MODEL_ID=$EXPECTED_REWARD_MODEL_ID $SCRIPT_DIR/run_hil_actor.sh --fake-env --no-classifier-sidecar"
echo "============================================================"
echo "LOCAL_SERVER_RUN_DIR=$RUN_DIR"
echo "LOCAL_SERVER_READY_LINE=$READY_LINE"
echo "LOCAL_SERVER_RESULT=started"
echo "Keep this terminal open.  Ctrl-C stops the FM server started above --"
...
SERVER_RC=$?
set -e
echo "FM policy server exited with status $SERVER_RC." >&2
exit "$SERVER_RC"
