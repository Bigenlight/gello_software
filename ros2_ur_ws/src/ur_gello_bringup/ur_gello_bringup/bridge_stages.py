...
joint-passthrough teleop, that loop is split into three ordered stages:
    ...
    (2) eef stage       -> ONLY when control_mode=="eef" and ENGAGED: overwrite
                           ``filtered`` with the EEF controller's slew-limited
                           q_cmd (or hold on None); a NO-OP in joint mode
...
def command_pipeline(
    ...
    Stage 2 (eef) runs ONLY when ``control_mode == "eef"`` and ``engaged``:
      * ``eef_command`` not None -> OVERWRITE ``filtered`` with the EEF
        controller's already-slew-limited q_cmd; the downstream clamp then does
        not bind (``step`` is passed to the EEF controller too).
      * ``eef_command`` is None  -> HOLD: set ``filtered`` to ``last_published``
        so the clamp delta is exactly zero (arm freezes).
    Stage 3 (clamp) ALWAYS runs.

    In joint mode stage 2 is a no-op, so the result equals
    ``clamp_stage(filter_stage_joint(...), last_published, step)`` — bit-identical
    to the legacy interleaved loop regardless of any ``eef_command`` argument.
    """
    filter_stage_joint(raw_target, filtered, gated_target, euro, ema_alpha, deadband)
    if control_mode == "eef" and engaged:
