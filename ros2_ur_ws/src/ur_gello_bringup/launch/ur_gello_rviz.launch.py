#!/usr/bin/env python3
"""Bring up a UR5e in RViz with mock/fake hardware and drive it from GELLO.

This launch file lets you VISUALIZE GELLO -> UR5e teleoperation in RViz
WITHOUT a real robot. It:

  1. Includes the official ``ur_control.launch.py`` from ``ur_robot_driver``
     with ``use_fake_hardware:=true``. The ros2_control mock hardware simply
     mirrors position commands back to the joint states, so any command sent
     to the ``forward_position_controller`` makes the RViz model move.
...
Example:
    ros2 launch ur_gello_bringup ur_gello_rviz.launch.py                # fake source
    ros2 launch ur_gello_bringup ur_gello_rviz.launch.py source:=gello  # real GELLO
"""
...
def generate_launch_description():
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            "ur_type",
            default_value="ur5e",
            description="Type/series of the UR robot (e.g. ur3e, ur5e, ur10e).",
        ),
        DeclareLaunchArgument(
            "robot_ip",
    ...
    ur_control_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            # older releases used 'use_fake_hardware'. Pass both — unknown
            # launch_arguments are silently ignored by the include.
