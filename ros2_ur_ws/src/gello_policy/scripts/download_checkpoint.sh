#!/usr/bin/env bash
# Download the trained ACT "put right banana in pot" checkpoint (and optionally the
# LeRobot dataset) from the Hugging Face Hub into a GITIGNORED local dir, then print
# the checkpoint path to pass to the ACT server as --checkpoint.
#
#   ACT checkpoint : Bigenlight/act_banana_in_pot
...
# #    WITH_DATASET=1 ./download_checkpoint.sh                              #
# #                                                                          #
# #    # Use it directly:                                                   #
# #    CKPT=$(./download_checkpoint.sh | tail -1)                           #
# #    ACT_CHECKPOINT="$CKPT" ./run_act_server.sh                           #
# #                                                                          #
# #  ENV:                                                                    #
...
# #    CKPT_REPO    HF repo id       (default: Bigenlight/act_banana_in_pot) #
...
CKPT_REPO="${CKPT_REPO:-Bigenlight/act_banana_in_pot}"
...
CKPT_DIR="$DOWNLOAD_DIR/act_banana_in_pot"
mkdir -p "$CKPT_DIR"

echo "### Downloading ACT checkpoint '$CKPT_REPO' -> $CKPT_DIR" >&2
...
# The ACT server wants the pretrained_model dir if the repo nests one; otherwise
# the repo root IS the pretrained model. Print whichever exists (path on stdout).
...
echo "### Checkpoint ready. Pass this to --checkpoint / ACT_CHECKPOINT:" >&2
echo "$CKPT_PATH"
