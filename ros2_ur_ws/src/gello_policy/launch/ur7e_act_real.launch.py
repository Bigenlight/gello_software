#!/usr/bin/env python3
r"""Bring up a REAL UR7e and drive it AUTONOMOUSLY from the trained ACT policy.

    ############################################################################
    #  READ THIS BEFORE RUNNING ON REAL HARDWARE                               #
    #                                                                          #
    #  THE ACT POLICY DRIVES THE ARM AUTONOMOUSLY.                             #
    ...
    #    GELLO arm it queries the py3.12 ACT inference server over localhost    #
    #    ZMQ and publishes the policy's joint targets onto /gello/joint_states  #
    #    (+ the gripper). Everything downstream (gello_ur_bridge handshake,     #
    #    move-to-start, Robotiq Modbus) is UNCHANGED and unaware the leader is  #
    ...
    #    fixed start_pose (from act_deploy.yaml) perfectly still and does NOT   #
    #    query the server. gello_move_to_start therefore chases that constant   #
    #    start pose and, once converged (chase_tol, sustained chase_dwell_s),   #
    #    STRICT-switches to forward_position_controller and resumes the bridge  #
    #    exactly as in the human-GELLO launch. NO autonomous motion happens     #
    #    during the handshake — the leader is holding still.                    #
    #                                                                          #
    #  THE OPERATOR MUST EXPLICITLY START EXECUTION:                           #
    #    Autonomous ACT motion begins ONLY when the operator calls             #
    #        ros2 service call /policy_leader_node/start_execution \            #
    #            std_srvs/srv/Trigger                                           #
    #    (unless auto_start_on_stream:=true in the params file). On start the   #
    #    leader RESETs the server and enters EXECUTE: each tick it queries ACT, #
    #    SAFETY-CLAMPS the returned target (1.2x joint envelope, then per-joint #
    #    max-deviation from the live pose) and publishes it. On any ZMQ         #
    #    timeout / server error it FAULTS: it STOPS publishing, so the bridge's #
    #    0.5 s staleness watchdog halts the arm (fail-silent — never a stale    #
    #    held target through an outage). Recover with ~/start_execution again.  #
    #                                                                          #
    #  Keep the teach-pendant E-STOP within reach at all times. Ctrl-C on the   #
    #  run script tears down BOTH the ACT server and this launch cleanly.       #
    ############################################################################

Bring-up sequence (staggered TimerActions) — identical topology to
ur7e_gello_real.launch.py, with the physical GELLO source swapped for the ACT
synthetic leader:
  ...
  (2) t=6s    policy_leader_node (SYNTHETIC ACT LEADER) -> publishes
...
Prerequisites:
    * The py3.12 ACT inference server must be reachable at act_host:act_port
      (start it first — run_ur7e_act_real.sh launches it automatically before
      this launch, or run scripts/run_act_server.sh standalone).
    * A trained checkpoint (Bigenlight/act_banana_in_pot; download via
      scripts/download_checkpoint.sh).

Example:
    ros2 launch gello_policy ur7e_act_real.launch.py robot_ip:=192.168.10.11
    ros2 launch gello_policy ur7e_act_real.launch.py \
        robot_ip:=192.168.10.11 act_port:=5591 \
...
def generate_launch_description():
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            ...
            description=(
                ...
                "only: full autonomous ACT execution ADDITIONALLY needs the two "
                "RealSense cameras running AND the gripper position topic faked "
        ...
        DeclareLaunchArgument(
            "params_file",
            # Default to THIS package's ACT deploy params (drives policy_leader_node,
            # gello_ur_bridge, gello_move_to_start and robotiq_gripper from one file).
            default_value=PathJoinSubstitution(
                [
            ...
            description=(
                "YAML parameter file for the ACT leader / bridge / move-to-start / "
                "gripper nodes. Defaults to gello_policy/config/act_deploy.yaml."
            ),
        ),
        DeclareLaunchArgument(
            "start_mode",
        ...
        DeclareLaunchArgument(
            ...
            description=(
                "Host of the py3.12 ACT inference server (ZMQ REQ/REP). Overrides "
                "policy_leader_node.act_host in the params file."
            ),
        ),
        DeclareLaunchArgument(
            "act_port",
            default_value="5591",
            description=(
                "TCP port of the ACT inference server. Overrides "
                "policy_leader_node.act_port in the params file. run_ur7e_act_real.sh "
                "passes the same port it started the server on."
            ),
        ),
        DeclareLaunchArgument(
            "checkpoint_path",
            default_value="",
            description=(
                "LOGGING ONLY. The ACT checkpoint the server was started with; this "
                "launch does NOT load the checkpoint (the separate py3.12 server "
                "does). Passed only so the effective checkpoint is echoed in the "
                "launch log for provenance."
            ),
        ),
        DeclareLaunchArgument(
            "start_pose",
            ...
            description=(
                ...
                "the held pose, edit act_deploy.yaml. If set here it is only echoed "
                "in the launch log so the intended pose is visible."
            ),
        ),
    ]
    ...
    #     Identical to ur7e_gello_real.launch.py — the ACT deploy reuses the arm
    #     stack unchanged (trajectory controller ACTIVE for the handshake;
    ...
    # ------------------------------------------------------------------ #
    # (2) SYNTHETIC ACT LEADER (this package). Replaces gello_publisher: instead
    #     of reading a physical GELLO arm it queries the py3.12 ACT server over
    #     ZMQ and publishes the policy's targets on /gello/joint_states (+ the
    #     gripper command DIRECTLY). In HOLD it publishes its fixed start_pose so
    #     the handshake below has a still target to chase; it only queries the
    #     server after the operator calls ~/start_execution (or auto_start_on_stream).
    #
    ...
    #     NOTE: gello_gripper_bridge is DROPPED for ACT — policy_leader_node
    ...
    provenance_banner = LogInfo(
        msg=[
            "ACT deploy: policy_leader_node will query the ACT server at ",
            act_host, ":", act_port,
                ...
                return [
                    LogInfo(
                        msg=(
                            ...
                            "parked on the held start pose. NOTE: autonomous ACT "
                            "execution additionally requires the two RealSense "
                            "cameras running AND the gripper position topic faked "
                            "(the gripper node was skipped, so "
            ...
            return [
                LogInfo(
                    msg=(
                        ...
                        "/tmp/ttyUR). The ACT leader publishes command_percent "
                        ...
                        "to begin AUTONOMOUS ACT motion."
                    )
                ),
                gripper_modbus_node,
            ]
        return [
            LogInfo(
                msg=(
                    ...
                    "robot will stay put and the ACT leader stays in HOLD. Check "
                    "that the External Control program is PLAYING on the pendant, "
                    "then re-launch."
                )
            )
        ]
