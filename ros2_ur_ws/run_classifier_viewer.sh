...
# Canonical checkpoint: the orbax *directory* holding checkpoint_150.  The old
# classifier_ckpt/cube_in_cup checkpoint is retired (0% held-out recall on the
# current domain) and is never used as a fallback.  Checkpoints stay out of git.
...
# DEFAULT_REWARD_THRESHOLD so the viewer verdict matches the learner's reward.
