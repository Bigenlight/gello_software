#!/usr/bin/env bash
# Launch live HIL-SERL reward inference and the existing policy-run GUI.
# Cameras must already publish cam1/cam2 compressed topics.
#
# Optional overrides:
...
# Use the exact upstream revision pinned by this repository.  HIL_SERL_ROOT may
# still be overridden for development, but no sibling checkout is required.
export HIL_SERL_ROOT="${HIL_SERL_ROOT:-$REPO_ROOT/third_party/hil-serl}"
# The learned task checkpoint lives inside this working tree but is excluded
# from git.  Canonical = the orbax *directory* cube_in_cup_all3 (holds
# checkpoint_150).  The old cube_in_cup checkpoint is retired: 0% held-out
# recall on the current domain, so it is never a fallback.
export REWARD_CLASSIFIER_CHECKPOINT="${REWARD_CLASSIFIER_CHECKPOINT:-$REPO_ROOT/classifier_ckpt/cube_in_cup_all3}"
...
if ! kill -0 "$CLASSIFIER_PID" 2>/dev/null; then
    echo "reward_classifier exited; closing policy GUI (exit=$FIRST_EXIT)." >&2
