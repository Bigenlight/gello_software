...
| 녹화 take를 **Hugging Face 데이터셋으로 공개** (raw + LeRobot, **depth 포함**) | [`docs/ros2/GELLO_UR7E_RECORDING.md`](docs/ros2/GELLO_UR7E_RECORDING.md) 「허깅페이스 업로드」절 — 스크립트는 `scripts/dataset/`(변환·독립 검증·raw 통계). 📌 2026-09-14 `Bigenlight/carrot_in_pot_raw` · `carrot_in_pot_lerobot_v3`(54 에피소드, EEF 모드, depth = lerobot 네이티브 depth video, 선형 12-bit 0~10 m ±1.25 mm, 무효 0 보존). lerobot 기본 log 양자화는 0→10 mm로 바꿔 무효 마스크를 잃으니 쓰지 말 것. 🔴 **같은 날 밤 발견: 그 take들의 `ur_joint_states`가 0.900 s, `tcp_pose`/`wrench`가 ≈0.5 s 늦게 찍혀 있었다** — 레코더가 콜백 수신 시각을 `t_rel_s`로 쓰는데 depth 쓰기가 단일 spin 스레드를 굶겨 KEEP_LAST 큐(깊이 100/50)가 항상 찬 채로 돈 것. raw는 그대로(카드에 보정 규칙), LeRobot은 `observation.state`만 take별 τ(0.895~0.900 s)로 보정해 재발행(검증 79/79). 레코더는 헤더 스탬프 `stamp_s` 저장 + 큐 축소 + 쓰기 스레드 + 기아 경고로 고쳤고, **depth는 기본 OFF(`ENABLE_DEPTH=1`로만)** — `GELLO_UR7E_RECORDING.md` 「타임스탬프 아티팩트」절. ✅ 2026-09-16 수정된 레코더로 찍은 첫 릴리스 `Bigenlight/orange_bowl_in_purple_bowl_raw`(52 take, RGB만): `stamp_s` 있음, 관절 지연 0.14 s = 실제 서보 추종(빼면 안 됨), 기아 0/52 → **보정 불필요**. 통계는 태스크 무관 `scripts/dataset/make_raw_stats.py`, LeRobot 변환은 `convert_takes_to_lerobot.py`(stamp_s 정렬, τ 없음) + `validate_takes_conversion.py`(43/43) → `orange_bowl_in_purple_bowl_lerobot_v3`. 레코더 GUI에 **Delete last take** 버튼(마지막에 Stop한 take만, 비모달 확인, 번호 재사용)도 같은 날 추가 |
...
| GELLO로 실기 팔 텔레옵 (HIL 개입이 이 경로 위에 있다) | [`docs/ros2/GELLO_UR7E_EEF_MODE.md`](docs/ros2/GELLO_UR7E_EEF_MODE.md) · 조인트 모드는 [`GELLO_UR7E_REAL_ROBOT.md`](docs/ros2/GELLO_UR7E_REAL_ROBOT.md). 📌 2026-09-16 `run_eef_gui.sh`에 **GO TO START POSE** 버튼: `START_POSE_CONFIG`(기본 `gello_policy/config/ifql_deploy.yaml`)의 `policy_leader_node.start_pose`로 팔을 보낸다 — 추론 시작 포즈와 같은 정의를 읽으므로 데이터 수집 시작 포즈가 추론과 일치한다. 시퀀스는 task recorder GO HOME과 동일 기계(`gello_recorder/home_move.py` + `home_move_ros.py`, 브릿지 pause → STJC → 궤적 → 그리퍼 열기 → FPC 복귀, 자동 재개 없음). 🟡 실기 미검증 |
...
| **시뮬레이션에서 GELLO로 데이터 수집** (실기 레코더와 같은 형식의 take 녹화) | 현황·기능 요약은 [`sim_collect/STATUS.md`](sim_collect/STATUS.md). 런북 [`sim_collect/README.md`](sim_collect/README.md) — `./sim_collect/run_sim_collect.sh` 한 줄. MuJoCo UR7e + EEF 델타 제어 + cam1/cam2 + 조작자 GUI. 설계 계약은 [`sim_collect/DESIGN.md`](sim_collect/DESIGN.md). ⚠️ 플레이그라운드(`experiments/launch_yaml.py`)와 **GELLO 포트를 공유하므로 동시 실행 금지**. 🔴 실제 GELLO 텔레옵·파지는 **조작자 미검증** |
...
- **그리퍼 값 규약은 두 개고 서로 반대다 — 뒤집으면 에러 없이 정확히 거꾸로 동작한다.**
  토픽과 녹화 컬럼(`gello_grip`/`grip_cmd`/`grip_pos`, `/robotiq_gripper/*_percent`)은
  **0.0=열림 / 1.0=닫힘**이고, RL 액션 `action[6]`은 **+1=열림 / −1=닫힘**이다(0=유지).
  두 값 범위가 모두 유효해서 **런타임에 아무 경고도 나지 않는다.** 뒤집기는
  `recorded_demo.py`의 오프라인 변환에서만 일어난다 — **녹화는 언제나 0/1 규약이다.**
  레코더가 ROS 토픽을 그대로 적어야 ACT/Diffusion/FM과 LeRobot 내보내기(둘 다 `grip_cmd`
  0~1을 씀)가 안 깨지므로 **레코더를 ±1로 바꾸지 말 것.**
- **그리퍼 이산화 임계값은 `0.3 / 0.7`(리더 방아쇠, 히스테리시스)이고 새로 정하지 말 것.**
  `>=0.7 → 닫힘`, `<=0.3 → 열림`, 사이는 **직전 상태 유지**. `recorded_demo.py:559-570`이
  이미 쓰고 있고 실시간 discrete 모드도 같은 값을 재사용한다. take 125개 / 52,607 샘플
  전수 검증에서 **정지 구간이 중간값에 놓인 사례는 0건**이다(근거·날짜별 여유표:
  [`docs/ros2/GELLO_UR7E_UNITS_REFERENCE.md`](docs/ros2/GELLO_UR7E_UNITS_REFERENCE.md) §5.1).
  🪤 **이 임계값이 존재하는 이유**: 방아쇠가 **세션당 1~2회** 덜 열린 채로 멈춘다(07-20 최악
  **0.243**, 여유 0.057). **캘리브레이션 문제가 아니다** — 125개 take 전부에서 방아쇠는
  `0.000`에 도달하므로 바닥은 멀쩡하고, 높은 값은 산발적 사건이다. 그래도 GELLO가 그 값을
  내보내면 그리퍼는 실제로 덜 열리고, 이산화가 그것을 0.0으로 스냅해 고친다. 관측된 이상값은
  **전부 0.3 아래**여서 임계값이 이것을 잡는다(0.3~0.5 정지 사례 0건). 재캘리브레이션 후에는
  §5.1 표를 다시 재라.
- **`grip_pos`(측정 위치)를 이산화하거나 버리지 말 것.** 물체를 쥐면 손가락이 멈춰 실측
  최댓값이 **0.506**이다(빈 손 완전 닫힘은 ~0.90). 즉 위치만으로는 **잡았다와 헛잡았다를
  구분할 수 없고**, 이 연속 신호가 그 정보를 담은 유일한 채널이다. 이산화는 **명령
  (`grip_cmd`)에만** 적용한다.
- **그리퍼 셋포인트는 한 번만 보내면 안 된다 — 그리고 이 결함은 드라이버에서 안 보인다.**
  2026-08-06 실기: 방아쇠가 `0.00`인데 그리퍼가 0.42에 서 있었고, **닫기/열기 14회 중
  7회가 중간에서 멈췄다**(0.0431/0.0549/0.1098. 완전 열림은 0.0118). 원인은 셋 겹이다 —
  브릿지는 **값이 바뀔 때 한 번만** 발행하고(타이머 없음. `publish_rate_hz`는 읽히지 않는
  죽은 파라미터였다), 드라이버는 직전 채택 후 **50 ms 안에 온 셋포인트를 버렸고**
  (`command_rate_hz: 20` — 30 Hz 스트림의 **마지막** 값, 즉 "완전 열림"이 정확히 그 창에
  떨어진다), **아무도 재발행하지 않는다.**
  🪤 **드라이버에 "명령 vs 실제" 워치독을 달아 고치려 하지 말 것 — 불일치가 안 보인다.**
  rate limit으로 거부된 셋포인트는 `_last_cmd_pct`를 갱신하지 않으므로 드라이버 관점에서는
  자기가 쓴 값에 그리퍼가 정확히 가 있다. 어긋난 것은 **발행자의 의도뿐**이다. 수정은
  드라이버의 rate limiter가 **버리는 대신 최신 값을 합치는 것**(coalesce)이고, 그 한 곳이
  텔레옵·GO HOME·RL·정책 배포를 전부 덮는다.
  **닫힘이 멀쩡해 보이는 것에 속지 말 것** — 손가락이 어차피 ~0.898에서 멈추므로 닫기는
  마지막 몇 %가 유실돼도 결과가 같다. 같은 확률로 씹히는데 여는 쪽만 눈에 보인다.
  ⚠️ **이산 모드(0.3/0.7)는 이것을 안 고친다** — 그건 방아쇠 노이즈용이고, 오히려 발행이
  더 드물어 같은 위험에 그대로 노출된다. 두 결함이 똑같이 "덜 열림"으로 보인다:
  `trig`가 0.2대면 방아쇠 문제, `trig`가 0.00인데 덜 열렸으면 이 문제다.
  근거·실험 로그: [`docs/ros2/GELLO_UR7E_GRIPPER.md`](docs/ros2/GELLO_UR7E_GRIPPER.md) §9,
  rosbag은 `ros2_ur_ws/gello_logs/diag_gripper_halfopen_20260806_173307/`.
